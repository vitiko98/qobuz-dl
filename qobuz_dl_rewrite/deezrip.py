...
class Deezrip:
    ...
    def downloadTrack(self, r):
        ...
        try:
            size = round(
                8
                * int(get(download_url, stream=True).headers["Content-length"])
                / 1000000,
                2,
            )
        except Exception:
        ...
        return {
            ...
            "size": size,
            "speed": round(size / time_elapsed, 2),
        }
    ...
    def rip(self, query):
        ...
        for item in tracks:
            ...
            print(
                f"Downloading {track_title} ({tracks.index(item) + 1}/{len(tracks)})",
                end=" ",
            )
            print(item["link"])
        ...
        f_time = self.formatTime(total_time)
